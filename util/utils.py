...
def evaluation_pixel(clip_model: CLIP, dataset_name, dataloader, args, device):
    ...
    with torch.no_grad():
        for items in tqdm(dataloader):
            ...
            predict_masks = F.interpolate(predict_masks, size=(imgs.size(-2), imgs.size(-1)),
                                          mode='bilinear').cpu().numpy()
            sigma = 0.5 if dataset_name == "mvtec" else 1.0
            predict_masks = np.stack([gaussian_filter(mask, sigma=sigma) for mask in predict_masks])
...
def eval_all_class(clip_model: CLIP, dataset_name, test_dataset, args, logger, device):
    ...
    if args.vis == 0:
        ...
        return average_res  # ✅ 新增：把平均结果返回给调用者

    return None  # ✅ 新增：vis!=0 时没有数值可统计
